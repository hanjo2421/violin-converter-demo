...
SELECTABLE_INSTRUMENTS = [
    ...
    "clarinet", "viola", "bass", "horn", "trombone"
]

INSTRUMENT_KEYWORD_MAP: Dict[str, List[str]] = {
    "violin": VIOLIN_KEYWORDS,
    "cello": ["cello", "vc.", "violoncello"],
    "flute": ["flute", "fl."],
    "trumpet": ["trumpet", "tpt."],
    "clarinet": ["clarinet", "cl."],
    "viola": ["viola", "vla."],
    "bass": ["double bass", "contrabass", "cb.", "bass"],
    "horn": ["horn", "hn."],
...
class ScoreAnalyzer:
    def __init__(self, img_path, instrument="violin", debug=False):
    ...
    def detect_parts(self):
        """Detect requested instrument parts via OCR."""
        if not self.staffs:
            logging.error("No staffs detected, cannot find instrument parts")
            return self

        keywords = INSTRUMENT_KEYWORD_MAP.get(self.instrument, [self.instrument])
        margin_width_candidates = [0.2, 0.3, 0.4]
        hit_positions = []

        ocr_configs = ['--psm 6 -l eng', '--psm 4 -l eng', '--psm 7 -l eng']

        h, w = self.original_size
        for frac in margin_width_candidates:
            margin = self.original_gray[:, : int(w * frac)]
            margin_enh = self._enhance_image(margin)
            _, margin_bin = cv2.threshold(margin_enh, 127, 255, cv2.THRESH_BINARY)

            for cfg in ocr_configs:
                data = pytesseract.image_to_data(margin_bin, config=cfg,
                                                output_type=pytesseract.Output.DICT)
                for txt, top, height, conf in zip(data['text'], data['top'],
                                                data['height'], data['conf']):
                    if conf > OCR_CONFIDENCE_THRESHOLD:
                        txt_low = txt.lower().strip()
                        if any(k in txt_low for k in keywords):
                            cy = top + height / 2
                            hit_positions.append(cy)
                            logging.info(f"OCR found '{txt_low}' at y={cy}")

            if hit_positions:
                break

        if not hit_positions:
            logging.warning("No matching instrument label detected; using all staffs.")
            self.violin_staffs = self.staffs
            return self

        matched = []
        for pos in hit_positions:
            nearest = min(self.staffs, key=lambda s: abs(s.cy - pos))
            matched.append(nearest)
        self.violin_staffs = list({s for s in matched})

        for s in self.violin_staffs:
            s.draw(self.debug_img, (0, 255, 0), 2)
        self.debug_images["instrument_parts"] = self.debug_img.copy()
        return self
    ...
    def generate_midi(self, output_path):
        ...
        # Create MIDI file
        midi = MIDIFile(1)
        midi.addTrackName(0, 0, self.instrument.capitalize())
...
def convert(img_path, out_mid, instrument="violin",
            tmpl_dir='resources/template', debug=False, force_all=False):
    """Main conversion function: Sheet music to Violin MIDI"""
    try:
        # Create analyzer and process the image
        analyzer = ScoreAnalyzer(img_path, instrument=instrument, debug=debug)
        analyzer.preprocess(auto_crop=True)
        analyzer.detect_staffs()
        if force_all:
            analyzer.detect_violin_parts_force_all()  
        else:
            analyzer.detect_parts()                 
...
def main():
    ...
    parser.add_argument('--inst', default='violin', help=f"Target instrument ({', '.join(SELECTABLE_INSTRUMENTS)})")
    ...
    result = convert(args.image, args.output, instrument=args.inst, tmpl_dir=args.tmpl, debug=args.debug, force_all=args.force_all)
